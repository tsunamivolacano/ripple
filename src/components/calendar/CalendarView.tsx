...
export const CalendarView: React.FC<CalendarViewProps> = ({
  ...
  const getItemsForDate = (date: Date) => {
    const dayTasks = tasks.filter((t) => {
      ...
      if (filterType === 'exams') {
        ...
        return false; // Show only timetable slots
      }

      return doesTaskOccurOnDate(t, date);
    });

    const daySlots = filterType === 'exams' || filterType === 'assignments' || filterType === 'study_sessions'
      ? []
  ...
  return (
    <div className="space-y-6">
      ...
      /><dyad-write path="src/components/calendar/CalendarView.tsx" description="Completing CalendarView component">
import React, { useState, useMemo } from 'react';
import { useRipple } from '@/context/RippleContext';
import { Task, TimetableSlot } from '@/types/ripple';
import { CalendarHeader, ViewMode, FilterType } from './CalendarHeader';
